<template>
    ...
    <button @click="agregarUsuario" class ="btn btn-primary my-3">
        Agregar usuario
    </button>
    <p v-if="usuarios.length === 0" class="alert alert-warning">No hay usuarios disponibles</p>
    <v-else>
        <ul class="list-group">
    ...
    <v-else>
        ...
        <tbody table-striped >
            <tr scope="row" v-for="usuario in usuarios" :key="usuario.id">
                ...
                <td>{{ usuario.edad }}</td>
                <td>
                  <input type="checkbox" :checked="usuario.curso" @change="usuario.curso = !usuario.curso">
                </td>
...
const agregarUsuario = () => {
    const nuevoUsuario = {
        id: Date.now(),
        nombre: `Usuario ${usuarios.value.length + 1}`,
        edad: Math.floor(Math.random() * 30) + 20,
        curso: Math.random() > 0.5,
